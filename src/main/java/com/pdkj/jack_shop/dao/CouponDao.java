...
public class CouponDao extends DaoBase {
    ...
    public List<Map<String, Object>> getCouponByUserId(Long user_id,Pager pager) {
        ...
        sql.append(" uo.user_id = ? AND uod.type_of_id = 1 AND uod.order_state_id = 2",  user_id);
        sql.append("order by c.coupon_state asc, uod.order_state_id asc,uo.created desc");
        sql.limit(pager);
        return jdbcTemplate.queryForList(sql.toString(), sql.getValues());
    }
    ...
    public Map<String , Object> verifyCoupon(Long item_id){
        ...
        mySql.append("where");
        mySql.append("c.shop_id = s.id AND c.id = ? AND coupon_state = 1 ",item_id);
